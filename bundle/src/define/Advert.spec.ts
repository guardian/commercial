...
describe('Advert', () => {
	...
	describe('globalAdEvents', () => {
		it('should dispatch adCreated on globalAdEvents when created', () => {
			const listener = jest.fn();
			globalAdEvents.addEventListener('adCreated', listener);
			...
			expect(
				// eslint-disable-next-line @typescript-eslint/no-unsafe-member-access -- test
				(listener.mock.calls[0][0] as CustomEvent).detail.advert,
			).toBeInstanceOf(Advert);

			globalAdEvents.removeEventListener('adCreated', listener);
		});
	});
