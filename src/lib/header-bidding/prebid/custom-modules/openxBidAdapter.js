...
import { mergeDeep } from 'prebid.js/src/utils.js';
import { BANNER, NATIVE, VIDEO } from 'prebid.js/src/mediaTypes.js';
import { ortbConverter } from 'prebid.js/libraries/ortbConverter/converter.js';
...
const converter = ortbConverter({
	...
	imp(buildImp, bidRequest, context) {
		...
		if (bidRequest.params.customParams) {
			utils.deepSetValue(
				imp,
				'ext.customParams',
				bidRequest.params.customParams,
			);
		}
	...
	request(buildRequest, imps, bidderRequest, context) {
		...
		if (bid.params.response_template_name) {
			utils.deepSetValue(
				req,
				'ext.response_template_name',
				bid.params.response_template_name,
			);
		}
		if (bid.params.test) {
			req.test = 1;
		...
		if (ortbRequest.ext) {
			...
			if (ortbRequest.ext.platform) {
				utils.deepSetValue(
					ortbResponse,
					'ext.platform',
					ortbRequest.ext.platform,
				);
			}
		}
		const response = buildResponse(bidResponses, ortbResponse, context);
		// TODO: we may want to standardize this and move fledge logic to ortbConverter
		let fledgeAuctionConfigs = utils.deepAccess(
			ortbResponse,
			'ext.fledge_auction_configs',
		);
		if (fledgeAuctionConfigs) {
			fledgeAuctionConfigs = Object.entries(fledgeAuctionConfigs).map(
				([bidId, cfg]) => {
					return {
						bidId,
...
function isBidRequestValid(bidRequest) {
	const hasDelDomainOrPlatform =
		bidRequest.params.delDomain || bidRequest.params.platform;

	if (
		utils.deepAccess(bidRequest, 'mediaTypes.banner') &&
		hasDelDomainOrPlatform
	) {
		return (
			!!bidRequest.params.unit ||
...
function buildRequests(bids, bidderRequest) {
	...
	let bannerAndNativeBids = bids
		.filter((bid) => isBannerBid(bid) || isNativeBid(bid))
		// In case of multi-format bids remove `video` from mediaTypes as for video a separate bid request is built
		.map((bid) => ({
			...bid,
			mediaTypes: { ...bid.mediaTypes, video: undefined },
		}));

	let requests = bannerAndNativeBids.length
		? [createRequest(bannerAndNativeBids, bidderRequest, null)]
		: [];
