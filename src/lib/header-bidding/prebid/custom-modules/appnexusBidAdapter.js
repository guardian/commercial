import {
	...
	logWarn,
	mergeDeep,
} from 'prebid.js/src/utils.js';
import { Renderer } from 'prebid.js/src/Renderer.js';
import { config } from 'prebid.js/src/config.js';
import { registerBidder } from 'prebid.js/src/adapters/bidderFactory.js';
import { ADPOD, BANNER, NATIVE, VIDEO } from 'prebid.js/src/mediaTypes.js';
import { find, includes } from 'prebid.js/src/polyfill.js';
import { INSTREAM, OUTSTREAM } from 'prebid.js/src/video.js';
import { getStorageManager } from 'prebid.js/src/storageManager.js';
import { bidderSettings } from 'prebid.js/src/bidderSettings.js';
import { hasPurpose1Consent } from 'prebid.js/src/utils/gdpr.js';
import { convertOrtbRequestToProprietaryNative } from 'prebid.js/src/native.js';
import { APPNEXUS_CATEGORY_MAPPING } from 'prebid.js/libraries/categoryTranslationMapping/index.js';
import {
	convertKeywordStringToANMap,
	getANKewyordParamFromMaps,
	getANKeywordParam,
} from 'prebid.js/libraries/appnexusUtils/anKeywords.js';
import {
	convertCamelToUnderscore,
	fill,
	appnexusAliases,
} from 'prebid.js/libraries/appnexusUtils/anUtils.js';
import { convertTypes } from 'prebid.js/libraries/transformParamsUtils/convertTypes.js';
import { chunk } from 'prebid.js/libraries/chunk/chunk.js';
...
const VIEWABILITY_URL_START =
	/\/\/cdn\.adnxs\.com\/v|\/\/cdn\.adnxs\-simple\.com\/v/;
...
export const spec = {
	...
	buildRequests: function (bidRequests, bidderRequest) {
		...
		if (
			appDeviceObjBid &&
			appDeviceObjBid.params &&
			appDeviceObjBid.params.app
		) {
			appDeviceObj = {};
			Object.keys(appDeviceObjBid.params.app)
				...
				.forEach(
					(param) =>
						(appDeviceObj[param] =
							appDeviceObjBid.params.app[param]),
				);
		}
		...
		const member = memberIdBid
			? parseInt(memberIdBid.params.member, 10)
			: 0;
		...
		if (bidderRequest?.ortb2?.device) {
			...
			mergeDeep(
				payload.device,
				convertORTB2DeviceDataToAppNexusDeviceObject(
		...
		if (bidderRequest && bidderRequest.refererInfo) {
			let refererinfo = {
				...
				rd_ref: encodeURIComponent(
					bidderRequest.refererInfo.topmostLocation,
				),
		...
		if (FEATURES.VIDEO) {
			...
			if (hasAdPodBid) {
				bidRequests.filter(hasAdPod).forEach((adPodBid) => {
					...
					const nonPodTags = payload.tags.filter(
						(tag) => tag.uuid !== adPodBid.bidId,
					);
		...
		if (bidderRequest?.ortb2?.regs?.ext?.dsa) {
			...
			if (
				isArray(pubDsaObj.transparency) &&
				pubDsaObj.transparency.every((v) => isPlainObject(v))
			) {
				const tpData = [];
...
function strIsAppnexusViewabilityScript(str) {
	...
	let viewUrlStartInStr =
		regexMatchUrlStart != null && regexMatchUrlStart.length >= 1;

	let regexMatchFileName = str.match(VIEWABILITY_FILE_NAME);
	let fileNameInStr =
		regexMatchFileName != null && regexMatchFileName.length >= 1;
...
function newRenderer(adUnitCode, rtbBid, rendererOptions = {}) {
	...
	renderer.setEventHandlers({
		impression: () =>
			logMessage('AppNexus outstream video impression event'),
...
function newBid(serverBid, rtbBid, bidderRequest) {
	...
	if (FEATURES.VIDEO && rtbBid.rtb.video) {
		...
		switch (videoContext) {
			case ADPOD:
				const primaryCatId = APPNEXUS_CATEGORY_MAPPING[
					rtbBid.brand_category_id
				]
					? APPNEXUS_CATEGORY_MAPPING[rtbBid.brand_category_id]
					: null;
				...
				bid.video = {
					...
					durationSeconds: Math.floor(
						rtbBid.rtb.video.duration_ms / 1000,
					),
					dealTier,
				};
			...
			case OUTSTREAM:
				...
				if (rtbBid.renderer_url) {
					...
					if (!rendererOptions) {
						rendererOptions = deepAccess(
							videoBid,
							'renderer.options',
						); // second the adUnit definition has preference (shouldn't options be .config?)
					}
...
function bidToTag(bid) {
	...
	if (bid.params.position) {
		...
		let mediaTypePos =
			deepAccess(bid, `mediaTypes.banner.pos`) ||
			deepAccess(bid, `mediaTypes.video.pos`);
	...
	const auKeywords = getANKewyordParamFromMaps(
		convertKeywordStringToANMap(
			deepAccess(bid, 'ortb2Imp.ext.data.keywords'),
		),
		bid.params?.keywords,
	);
	...
	if (
		FEATURES.NATIVE &&
		(bid.mediaType === NATIVE || deepAccess(bid, `mediaTypes.${NATIVE}`))
	) {
	...
	if (FEATURES.VIDEO) {
		...
		if (bid.params.video) {
			...
			if (
				bid.params.video.frameworks &&
				isArray(bid.params.video.frameworks)
			) {
					...
					switch (param) {
						...
						case 'skip':
							if (typeof tag.video['skippable'] !== 'boolean')
								tag.video['skippable'] =
									videoMediaType[param] === 1;
							break;
						case 'skipafter':
							if (typeof tag.video['skipoffset'] !== 'number')
								tag.video['skippoffset'] =
									videoMediaType[param];
							break;
						case 'playbackmethod':
							if (
								typeof tag.video['playback_method'] !== 'number'
							) {
						...
						case 'api':
							if (
								!tag['video_frameworks'] &&
								isArray(videoMediaType[param])
							) {
								...
								let apiTmp = videoMediaType[param]
									.map((val) => {
										let v =
											val === 4 ? 5 : val === 5 ? 4 : val;
						...
						case 'placement':
							if (typeof tag.video.context !== 'number') {
								...
								const contextVal =
									...
									getContextFromStartDelay(startdelay);
								tag.video.context =
									VIDEO_MAPPING.context[contextVal];
							}
							break;
					}
				});
...
function hasOmidSupport(bid) {
	...
	if (
		!hasOmid &&
		videoParams &&
		videoParams.frameworks &&
		isArray(videoParams.frameworks)
	) {
...
function createAdPodRequest(tags, adPodBid) {
	const { durationRangeSec, requireExactDuration } =
		adPodBid.mediaTypes.video;

	const numberOfPlacements = getAdPodPlacementNumber(
		adPodBid.mediaTypes.video,
	);
...
function getAdPodPlacementNumber(videoParams) {
	const { adPodDurationSec, durationRangeSec, requireExactDuration } =
		videoParams;
	const minAllowedDuration = Math.min(...durationRangeSec);
	const numberOfPlacements = Math.floor(
		adPodDurationSec / minAllowedDuration,
	);
...
function buildNativeRequest(params) {
	...
	Object.keys(params).forEach((key) => {
		...
		const requiredParams =
			NATIVE_MAPPING[key] && NATIVE_MAPPING[key].requiredParams;
		...
		const isImageAsset = !!(
			requestKey === NATIVE_MAPPING.image.serverName ||
			requestKey === NATIVE_MAPPING.icon.serverName
		);
		if (isImageAsset && request[requestKey].sizes) {
			...
			if (
				...
				(isArray(sizes) &&
					sizes.length > 0 &&
					sizes.every((sz) => isArrayOfNums(sz)))
			) {
				request[requestKey].sizes = transformSizes(
					request[requestKey].sizes,
				);
			}
		}
