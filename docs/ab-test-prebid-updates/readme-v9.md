...
Copy the folder `bundle/src/lib/header-bidding/prebid/modules` with a new name, e.g., `modules-v9.46.0`, and update all imports to use the new prebid alias.
...
if (shouldLoadPrebid946) {
	await import(
		...
		`../../lib/header-bidding/prebid/modules-v9.46.0`
	);
} else {
	await import(
